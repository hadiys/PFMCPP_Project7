...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    {
        //same with paladins
        pl->boostArmor( item.getBoost() * 1.3 );
    }
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        pl->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
...
void useAttackItem(Character* character, Item* item)
{
    ...
    {
        pl->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
        //assert(false);
        ...
        int boost = ds->getAttackDamage() * item->getBoost();
        ds->boostAttackDamage(boost);
    }
    else if( auto* ch = dynamic_cast<Dragon*>(character) )
    {
